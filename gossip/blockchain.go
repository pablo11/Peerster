...
type Blockchain struct {
	...
	// Mapping of votation_id to array of VotationReplyWrapped in the blockchain [vote_id: string => [holderName: string => votationAnswerWrapper: *VotationAnswerWrapper]]
...
// This function checks that there are no two identities trying to claim the same name in the transactions list
...
func (b *Blockchain) HandlePktBlockPublish(gp *model.GossipPacket) {
	...
	// The integration of the transactions shouldn't be done here but only when a
	// block is to be added on the longest chain, otherwise we update the state of
	// the blockchain even if it's a block of a shorter chain
...
func (b *Blockchain) SendIdentityTx(identityName string) {
	...
	if identityName == b.gossiper.Name {
		// Identity using THIS peer name
		privateKey = b.gossiper.PrivateKey
	} else {
		// Identity using a name differing from this peer's one (can be created via client for test purposes)
	...
	if isValid {
		if b.isAlreadyPendingIdentity(newIdentity) {
